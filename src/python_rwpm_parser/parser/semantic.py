...
from python_rwpm_parser.ast import (Collection, CollectionList, Link,
                                    LinksList, Manifestlike, Metadata, Visitor)
from python_rwpm_parser.registry import (RWPMCollectionRolesRegistry,
                                         RWPMLinkRelationsRegistry)
from python_rwpm_parser.utils import first_or_default


class SemanticError(Exception):
    """Raised in the case of any semantic errors"""


MISSING_TYPE_METADATA_ERROR = SemanticError('Manifest\'s metadata does not contain @type property')
MISSING_TITLE_METADATA_ERROR = SemanticError('RWPM Manifest\'s metadata does not contain title property')
...
class CollectionWrongFormatError(SemanticError):
    """Raised in the case when collection's format (compact, full) doesn't not conform with its role"""

    def __init__(self, collection, inner_exception=None):
        """Initializes a new instance of CollectionWrongFormat class
        ...
        message = 'Collection {0} must be {1} but it is not'.format(
            collection.role.key,
            'compact' if collection.role.compact else 'full'
        )

        super(CollectionWrongFormatError, self).__init__(collection, message, inner_exception)

        self._collection = collection

    @property
    def collection(self):
        """Returns a collection with a wrong format
...
class MissingCollectionError(SemanticError):
    def __init__(self, collection_role, inner_exception=None):
        ...
        :param collection_role: Role of the reqired collection
...
class LinkWrongFormatError(SemanticError):
    def __init__(self, link, message, inner_exception=None):
        """Initializes a new instance of LinkWrongFormatError class
        ...
        :type inner_exception: Optional[Exception]
        """
        super(LinkWrongFormatError, self).__init__(
            message,
            inner_exception
        )
    ...
    @property
    def link(self):
        """Returns a link with a wrong format
...
class MissingLinkTypeError(LinkWrongFormatError):
    """Raised when a link does not have type"""

    def __init__(self, link, inner_exception=None):
        """Initializes a new instance of MissingLinkTypeError class
        ...
        super(MissingLinkTypeError, self).__init__(
            link,
            'Link {0} does not have type'.format(link),
            inner_exception
        )


class UnknownLinkTypeError(LinkWrongFormatError):
    """Raised when a link has an unknown (not registered) type"""

    def __init__(self, link, inner_exception=None):
        """Initializes a new instance of UnknownLinkTypeError class
...
class SemanticAnalyzer(Visitor):
    ...
    def __init__(self, media_types_registry, link_relations_registry, collection_roles_registry):
        """Initializes a new instance of SemanticAnalyzer
    ...
    def visit(self, node):
        """Performs semantic analysis of the manifest node
        ...
        self_link = first_or_default(node.links.get_by_rel(RWPMLinkRelationsRegistry.SELF.key))

        if self_link is None:
            raise MISSING_SELF_LINK_ERROR

        if self_link.href is None or \
                (not self_link.href.startswith('http://') and
                 not self_link.href.startswith('https://')):
    ...
    @dispatch(Metadata)
    def visit(self, node):
        """Performs semantic analysis of the manifest's metadata
    ...
    @dispatch(LinksList)
    def visit(self, node):
        """Performs semantic analysis of the list of links
    ...
    @dispatch(Link)
    def visit(self, node):
        """Performs semantic analysis of the link node
    ...
    def visit(self, node):
        """Performs semantic analysis of the list of sub-collections
    ...
    @dispatch(Collection)
    def visit(self, node):
        """Performs semantic analysis of the collection node
        ...
        if node.role.compact:
            ...
            if node.role.key in [RWPMCollectionRolesRegistry.READING_ORDER.key,
...
class RWPMSemanticAnalyzer(SemanticAnalyzer):
    ...
    def __init__(self, media_types_registry, link_relations_registry, collection_roles_registry):
        """Initializes a new instance of RWPMSemanticAnalyzer class
