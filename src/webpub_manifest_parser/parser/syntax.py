...
from webpub_manifest_parser.metadata import PropertiesGrouping


class ParsingError(BaseError):
    """Exception raised in the case of parsing errors."""
...
class MissingSubCollectionError(ParsingError):
    """Exception raised in the case of a missing required sub-collection."""

    def __init__(self, sub_collection_role, inner_exception=None):
        """Initialize a new instance of MissingSubCollectionError class.
    ...
    def sub_collection_role(self):
        """Return the role of the missing sub-collection.
...
class UnknownCollectionRolesError(ParsingError):
    """Exception raised when non-registered collections roles are found in the document."""

    def __init__(self, unknown_sub_collection_keys, inner_exception=None):
        """Initialize a new instance of UnknownCollectionRolesError class.
        ...
        super(UnknownCollectionRolesError, self).__init__(
            "The following sub-collection keys are not registered: {0}".format(
                unknown_sub_collection_keys
            ),
            inner_exception,
        )
    ...
    def unknown_sub_collection_keys(self):
        """Return the list of sub-collection keys which are present in the document but not registered.
...
class WrongFormatError(ParsingError):
    """Exception raised in the case of wrong format."""
...
class SyntaxAnalyzer(object):
    ...
    def _parse_context(self, json_content):
        """Parse manifest's context into string.
    ...
    def _parse_metadata(self, json_content):
        """Parse collection's metadata into Python dictionary.
        ...
        self._logger.debug(
            "Finished parsing collection's metadata: {0}".format(metadata)
        )
        ...
        for object_property in ast_object_properties:
            ...
            self._logger.debug(
                "Property {0} has the following value: {1}".format(
                    object_property.key, property_value
                )
            )
    ...
    def _parse_links(self, json_content):
        """Parse links' JSON into a list of Link objects.
        ...
        for link in links:
            ...
            self._logger.debug(
                "Finished parsing link {0}: {1}".format(link, link_object)
            )
    ...
    def _parse_collection(self, json_content, collection=None, role=None):
        """Parse collection's JSON into a Collection object.
        ...
        """
        self._logger.debug(
            "Started parsing collection {0} with role {1}".format(collection, role)
        )
        ...
        if not role or not role.compact:
            ...
            sub_collection_keys = set(json_content.keys()) - {
                "@context",
                "metadata",
                "links",
            }
            ...
            self._logger.debug(
                "There are following potential sub-collections: {0}".format(
                    sub_collection_keys
                )
            )
            ...
            for collection_role in self._collection_roles_registry:
                if collection_role.key in sub_collection_keys:
                    sub_collection = self._parse_collection(
                        json_content[collection_role.key],
                        collection=None,
        ...
        self._logger.debug(
            "Finished parsing collection {0} with role {1}".format(collection, role)
        )

        return collection

    def _parse_manifest(self, json_content):
        """Parse manifest's JSON into manifest-like object.
    ...
    def analyze(self, input_file_path):
        ...
        with open(input_file_path, "r") as input_file:
            ...
            self._logger.debug(
                "Finished parsing input file {0}: {1}".format(input_file_path, manifest)
            )
