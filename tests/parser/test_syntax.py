...
from python_rwpm_parser.ast import Link, Metadata, RWPMManifest
from python_rwpm_parser.parser.syntax import (MISSING_CONTEXT_ERROR,
                                              MISSING_LINKS_ERROR,
...
from python_rwpm_parser.registry import (Registry, RWPMCollectionRolesRegistry,
...
class RWPMSyntaxAnalyzerTest(TestCase):
    @parameterized.expand([
        ('when_manifest_does_not_contain_context', RWPM_MANIFEST_WITHOUT_CONTEXT, MISSING_CONTEXT_ERROR),
        ('when_manifest_does_not_contain_metadata', RWPM_MANIFEST_WITHOUT_METADATA, MISSING_METADATA_ERROR),
        ('when_manifest_does_not_contain_links', RWPM_MANIFEST_WITHOUT_LINKS, MISSING_LINKS_ERROR)
    ])
    def test_syntax_analyzer_raises_correct_format_errors(self, _, rwpm_manifest_content, expected_error):
        # Arrange
        collection_roles_registry = Registry()
        syntax_analyzer = RWPMSyntaxAnalyzer(collection_roles_registry)
        expected_error_class = expected_error.__class__ if isinstance(expected_error, Exception) else expected_error
        expected_error_message = expected_error.message if isinstance(expected_error, Exception) else None
        ...
        with Patcher() as patcher:
            ...
            if expected_error_message:
                eq_(assert_raises_context.exception.message, expected_error_message)
    ...
    def test_syntax_analyzer_raises_missing_property_error(self):
        ...
        with Patcher() as patcher:
            patcher.fs.create_file(input_file_path, contents=RWPM_MANIFEST_WITH_MISSING_LINK_PROPERTY)
    ...
    def test_syntax_analyzer_raises_missing_sub_collection(self):
        ...
        collection_roles_registry = Registry([missing_sub_collection_role, RWPMCollectionRolesRegistry.PAGE_LIST])
        syntax_analyzer = RWPMSyntaxAnalyzer(collection_roles_registry)
        input_file_path = '/tmp/rwpm.jsonld'

        with Patcher() as patcher:
            patcher.fs.create_file(input_file_path, contents=RWPM_MANIFEST_WITH_MISSING_SUB_COLLECTION)
            ...
            # Assert
            eq_(assert_raises_context.exception.sub_collection_role, missing_sub_collection_role)
    ...
    def test_syntax_analyzer_raises_unknown_collection_error(self):
        ...
        with Patcher() as patcher:
            patcher.fs.create_file(input_file_path, contents=RWPM_MANIFEST_WITH_UNKNOWN_SUB_COLLECTION)
            ...
            # Assert
            eq_(assert_raises_context.exception.unknown_sub_collection_keys, ['unknownSubCollection'])
