...
from python_rwpm_parser.parser.semantic import (MISSING_SELF_LINK_ERROR,
...
from python_rwpm_parser.registry import (CollectionRole, Registry,
...
class SemanticAnalyzerTest(TestCase):
    @parameterized.expand([
        ...
        ('when_title_is_missing', {'@type': 'http://schema.org/Book'}, [], MISSING_TITLE_METADATA_ERROR),
        (
    ...
    def test_semantic_analyzer_raises_error(self, _, metadata, links, expected_error):
        ...
        # Assert
        eq_(assert_raises_context.exception.message, expected_error.message)
    ...
    @parameterized.expand([
        ...
        (
                ...
                [
                    Link(href='https://example.com/c001.html', type='text/unknown')
                ],
                UnknownLinkTypeError
        )
    ])
    def test_semantic_analyzer_raises_error_when_compact_collection_has(self, _, links, expected_error):
        # Arrange
        reading_order_sub_collection = Collection(RWPMCollectionRolesRegistry.READING_ORDER)
        reading_order_sub_collection.links.extend(links)

        self_link = Link(rel=RWPMLinkRelationsRegistry.SELF.key, href='https://example.org')
        ...
        media_types_registry = Registry()
        link_relations_registry = Registry()
        collection_roles_registry = Registry([RWPMCollectionRolesRegistry.READING_ORDER])
        semantic_analyzer = RWPMSemanticAnalyzer(
            media_types_registry, link_relations_registry, collection_roles_registry)
    ...
    def test_semantic_analyzer_raises_error_when_compact_collection_is_not_compact(self):
        # Arrange
        sub_collection_link_type = RWPMMediaTypesRegistry.HTML
        sub_collection_link = Link(href='https://example.org/index.html', type=sub_collection_link_type.key)
    ...
    def test_semantic_analyzer_does_correctly_processes_valid_ast(self):
        ...
        reading_order_sub_collection.links.append(
            Link(href='https://example.com/text.html', type=RWPMMediaTypesRegistry.HTML.key))

        self_link = Link(rel=RWPMLinkRelationsRegistry.SELF.key, href='https://example.com/manifest.json')
